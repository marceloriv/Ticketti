...
const ResumenCarrito = ({ resumen, onCheckout, loading, isGuest = false }) => {
  ...
  // Cargo por servicio fijo cuando hay items
  const cargoServicio = subtotal > 0 ? 3500 : 0;

  // Total siempre incluye la donación obligatoria del 10%
  const total = subtotal + cargoServicio + donacion;
  ...
  return (
    <div className="resumen-carrito-container card border-0 shadow-sm p-4">
      ...
      <div className="resumen-carrito-breakdown">
        ...
        <div className="resumen-carrito-fila">
          <span className="resumen-carrito-etiqueta">Cargo por Servicio</span>
          <span className="resumen-carrito-valor">{formatearMoneda(cargoServicio)}</span>
        </div>
        <div className="resumen-carrito-fila">
        ...
        <div className="resumen-carrito-total-row">
          <span className="resumen-carrito-total-etiqueta">Subtotal + Servicio</span>
          <span className="resumen-carrito-total-valor">
            {formatearMoneda(subtotal + cargoServicio)}
          </span>
        </div>
